...
function setTypeTag(type) {
  ...
  els.questionTypeTag.className = type === '单选'
    ? 'px-2.5 py-1 rounded-md text-[10px] font-black tracking-wider bg-blue-50 text-blue-600'
    : 'px-2.5 py-1 rounded-md text-[10px] font-black tracking-wider bg-orange-50 text-orange-600';
}
...
function renderFeedback(question) {
  ...
  els.feedbackBody.innerText = `参考答案：${question.answer}`;
}

function render() {
  const question = getCurrentQuestion();
  if (!question) return showSummary();
...
function submitAnswer(choice) {
  ...
  state.userChoices[state.currentIndex] = { choice, isCorrect };
  if (isCorrect) {
...
function submitMultiAnswer() {
  ...
  submitAnswer(normalizeAnswer(state.currentMultiSelection.join('')));
}
...
function showSummary() {
  ...
  els.summaryText.innerHTML = `共挑战 ${total} 道题目，<br>你答对了 <span class="text-green-600 font-bold">${state.stats.correct}</span> 题，正确率 <span class="text-indigo-600 font-black text-2xl">${rate}%</span>。`;
}
