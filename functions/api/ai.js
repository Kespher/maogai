const ACTION_CONFIG = {
  explain: {
    system: '你是一位金牌政治辅导老师。回答要准确、简洁、适合大学生考前复习。',
    buildPrompt(payload) {
      const q = payload.question;
      return [
        '请分析这道毛概题目，使用 Markdown 输出。',
        `题目：${q.stem}`,
  ...
  mnemonic: {
    system: '你擅长编押韵、好记、但不低幼的政治考点口诀。',
    buildPrompt(payload) {
      const q = payload.question;
      return [
        '请为这道毛概题编一个方便记忆的口诀，使用 Markdown 输出。',
        `题目：${q.stem}`,
        `正确答案：${q.answer}`,
        `选项：${q.options.map((item, index) => `${String.fromCharCode(65 + index)}. ${item}`).join('；')}`,
        '',
  ...
  report: {
    system: '你是一位备考规划师，擅长根据错题快速定位薄弱知识点。',
    buildPrompt(payload) {
      ...
      return [
        '请基于刷题记录生成一份毛概复习周报，使用 Markdown 输出。',
        `总题数：${payload.total}`,
...
export async function onRequestPost({ request, env }) {
  ...
  if (!env.GEMINI_API_KEY) {
    return json({
      error: 'AI 服务还没配置 GEMINI_API_KEY。静态刷题可用，AI 讲解需要在 Cloudflare 环境变量里添加密钥。'
    }, { status: 503 });
  }
  ...
  if (!upstream.ok) {
    return json({
      error: data.error?.message || 'Gemini API 请求失败，请检查模型名称、API Key 或账单状态。'
